...
void *read_thread(){
    for(;;){
        read_sound(capture_handle, alsa_buffer, ALSA_FRAMES);
        for(int j = FRAMES-1;j>=ALSA_FRAMES;j--){
            buffer[j] = buffer[j-ALSA_FRAMES];
        }
        for(int j = 0;j<ALSA_FRAMES;j++){
            buffer[j] = alsa_buffer[j];
        }
        new_stuff_to_render = 1;
    }
}

//Calculate values from the readings and renders them
void *calculate_and_render(){
    while(new_stuff_to_render == 0)
        usleep(1);
    new_stuff_to_render = 0;

    for(int j = 0;j<FRAMES;j++) {
        cpx_in[j] = (kiss_fft_cpx){.r = (buffer[j] - 22875746.0f)/100000000.0f, .i = 0};
    }
...
int main()
{
    ...
    alsa_buffer = malloc(ALSA_FRAMES * snd_pcm_format_width(format) / 8 * 2);
